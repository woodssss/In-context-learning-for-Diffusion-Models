...
# 6) Full model
# =========================================================
class SNOUNet(nn.Module):
    """
    Inputs:
      KME embedding: [B, embed]
      x_t:   [B,H,W,1]
      t:     [B]
    ...
    def __init__(
        ...
        mul_ls: List[int] = [1, 2, 4],
        pool_hidden: int = 128,
        kme_embed_dim: int = 64,       # dim of incoming KME embedding vector
        # self-attn configs
        ...
        super().__init__()
        temb_dim = base_ch * 4   # must match MultiResUNetPromptVector.temb_dim

        # Project KME embedding [B, kme_embed_dim] -> [B, temb_dim]
        self.kme_proj = nn.Sequential(
            nn.Linear(kme_embed_dim, temb_dim),
            nn.SiLU(),
    ...
    def forward(self, emb: torch.Tensor, x_t: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        emb: KME embedding [B, kme_embed_dim]  OR prompt images [N,H,W,C] / [B,N,H,W,C]
        x_t: [B, H, W, C]
        t:   [B]
        """
        B = x_t.shape[0]
        # If emb is a vector (KME path): project to temb_dim

        prompt_temb = self.kme_proj(emb)            # [B, temb_dim]
